...
class MaskLayer(nn.Module):
	...
	def mix(self, z):
		...
		if self.z1_dim == 1:
			...
			zys = [zy[:, i] for i in range(self.concept)]
			# if self.concept ==4:
			...
			# elif self.concept ==6:
			# 	zy1, zy2, zy3, zy4, zy5, zy6= zy[:,0],zy[:,1],zy[:,2],zy[:,3],zy[:,4],zy[:,5]
		else:
			zys = torch.split(zy, self.z_dim // self.concept, dim=1)
			# if self.concept ==4:
			...
			# elif self.concept == 6:
			# 	zy1, zy2, zy3, zy4, zy5, zy6 = torch.split(zy, self.z_dim // self.concept, dim=1)
		rxs = torch.stack([self.nets[i](zys[i]) for i in range(self.concept)], dim=0)
		# rx1 = self.net1(zy1)
		# rx2 = self.net2(zy2)
		# rx3 = self.net3(zy3)
		# if self.concept ==4:
		...
		# 	rx5 = self.net5(zy5)
		# 	rx6 = self.net6(zy6)
		# 	h = torch.cat((rx1,rx2,rx3,rx4,rx5,rx6), dim=1)
		h = torch.cat(tuple(rxs), dim=1)
		#print(h.size())
		return h
...
class Decoder_DAG(nn.Module):
	...
	def decode_sep(self, z, u, y=None):
		...
		# 	rx5 = self.net5(zy5)
		# 	rx6 = self.net6(zy6)
		# 	h = (rx1+rx2+rx3+rx4+rx5+rx6)/self.concept
		h = torch.sum(rxs, dim=0) / self.concept
	...
	def decode_cat(self, z, u, y=None):
		z = z.view(-1, 4*4)
		zy = z if y is None else torch.cat((z, y), dim=1)
		zy1, zy2, zy3, zy4, zy5, zy6 = torch.split(zy, 1, dim = 1)
		rx1 = self.net1(zy1)
		rx2 = self.net2(zy2)
		rx3 = self.net3(zy3)
		rx4 = self.net4(zy4)
		rx5 = self.net5(zy5)
		rx6 = self.net6(zy6)
		h = self.netx( torch.cat((rx1,rx2, rx3, rx4, rx5, rx6), dim=1))
		return h
